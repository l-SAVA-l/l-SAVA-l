# Приветствую! Я l-SAVA-l 👋

**Developer | Software Engineer**

Специализируюсь на разработке масштабируемых веб-приложений и кроссплатформенных решений. Имею опыт работы как с высокоуровневыми фреймворками (Next.js), так и с системно-ориентированным стеком (.NET/C#). Параллельно занимаюсь 3D-моделированием и техническим дизайном.

---

### 🛠 Стек технологий

#### **Frontend & UI/UX**
![Next.js](https://img.shields.io/badge/Next.js-black?style=flat-square&logo=next.js)
![React](https://img.shields.io/badge/React-20232A?style=flat-square&logo=react)
![Material UI](https://img.shields.io/badge/MUI-007FFF?style=flat-square&logo=mui)
![Tailwind](https://img.shields.io/badge/Tailwind-06B6D4?style=flat-square&logo=tailwind-css)

#### **Backend & Databases**
![C#](https://img.shields.io/badge/C%23-239120?style=flat-square&logo=c-sharp)
![.NET Core](https://img.shields.io/badge/.NET_Core-512BD4?style=flat-square&logo=dotnet)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=flat-square&logo=prisma)

#### **Tools & Design**
![Unity](https://img.shields.io/badge/Unity-FFFFFF?style=flat-square&logo=unity&logoColor=black)
![Blender](https://img.shields.io/badge/Blender-F5792A?style=flat-square&logo=blender)
![Adobe Illustrator](https://img.shields.io/badge/Illustrator-FF9A00?style=flat-square&logo=adobe-illustrator)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat-square&logo=docker)

---

### 🚀 Текущие проекты в разработке

* **Медицинская платформа (Full-stack):** Разработка системы управления записями пациентов с использованием Next.js, PostgreSQL и генерацией отчетности в PDF.
* **Web-каталог продукции:** Высокопроизводительный интерфейс на Material UI с адаптивной версткой и сложной фильтрацией данных.
* **Tactical Commander:** Пошаговая 2.5D стратегия на Unity с реализацией сетки и кастомных шейдеров.

---

### 📈 Статистика GitHub

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=ТВОЙ_ЛОГИН&show_icons=true&theme=transparent" alt="GitHub Stats" width="400"/>
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=ТВОЙ_ЛОГИН&layout=compact&theme=transparent" alt="Top Languages" width="300"/>
</p>

---

### 📫 Как со мной связаться

* **Email:** []
* **Telegram:** []
